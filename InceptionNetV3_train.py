...
transform = transforms.Compose([
    transforms.Resize((299, 299)),  # InceptionNet需要输入大小为(299, 299)
    transforms.ToTensor(),
...
# 使用预训练的InceptionNet模型
model = models.inception_v3(pretrained=True)

# 修改最后一个全连接层，以适应您的分类任务（3个类别）
num_features = model.fc.in_features
model.fc = nn.Linear(num_features, 3)  # 将3更改为您的类别数
...
weight_save_path = '../weights/inceptionnet/'
...
# 保存最佳权重
torch.save(best_model_weights, weight_save_path + 'inceptionnet_best_model_weights.pth')
